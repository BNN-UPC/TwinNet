...
NO_SOL_IT = 4
COMB_PER_ITERATION = 2000
MAX_SAMPLES = 5000
df_to_concat = []
# list(range(1900, 900, -100))

for intensity in list(range(2200, 1900, -100)):
    K = 1
    G = nx.DiGraph(sample.get_topology_object())
    R = sample.get_routing_matrix()
    routing = np.copy(R)
    ...
    with open('./optimizer/routing/routing_{}.pkl'.format(intensity), 'wb') as f:
        pickle.dump(routing, f, pickle.HIGHEST_PROTOCOL)
    with open('./optimizer/routing/delays_{}.pkl'.format(intensity), 'wb') as f:
        pickle.dump(delays, f, pickle.HIGHEST_PROTOCOL)
    with open('./optimizer/routing/ini_routing_{}.pkl'.format(intensity), 'wb') as f:
        pickle.dump(sample.get_routing_matrix(), f, pickle.HIGHEST_PROTOCOL)
    with open('./optimizer/routing/ini_delays_{}.pkl'.format(intensity), 'wb') as f:
        pickle.dump(ini_delays, f, pickle.HIGHEST_PROTOCOL)
