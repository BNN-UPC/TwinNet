...
with open('./optimizer/scheduling/scheduling_1500.pkl', 'rb') as f:
    G = pickle.load(f)
...
graphs = []
new_links = []
links_to_add = [(6, 16), (8, 13), (7, 15), (6, 14), (5, 12)]
graphs.append(G.copy())
new_links.append((-1, -1))
print("COMPUTING NEW GRAPHS")
for src, dst in links_to_add:
    if not G.has_edge(src, dst):
        G_aux = G.copy()
        G_aux.add_edge(src, dst, bandwidth=40000)
        G_aux.add_edge(dst, src, bandwidth=40000)
        graphs.append(G_aux)
        new_links.append((src, dst))

print("STARTING OPTIMIZER")
for tm in range(2, 11):
    with open('./optimizer/network_upgrade/TM_{}.pkl'.format(tm), 'rb') as f:
        T = pickle.load(f)
        sample._set_traffic_matrix(T)

    for it_g in range(len(graphs)):
        ...
        for intensity in [inten]:
            ...
            while True:
                if it_no_sol == NO_SOL_IT:
                    break
                print("ITERATION {} FOR TM {} AND GRAPH {}".format(it, tm, it_g))
                ...
                for i in range(len(splited_delay)):
                    ...
                    if len(s_no_sat_tos[0]) < len(no_sat_tos[0]):
                        ...
                        routing = np.copy(comb_routing[i])
                        it_no_sol = 0
                    elif (len(s_no_sat_tos[0]) == len(no_sat_tos[0])) and (
                            len(s_no_sat_tos[1]) == len(no_sat_tos[1])) and (
            ...
            with open('./optimizer/network_upgrade/routing_TM_{}_{}--{}.pkl'.format(tm, new_links[it_g][0],
                                                                                    new_links[it_g][1]), 'wb') as f:
                pickle.dump(routing, f, pickle.HIGHEST_PROTOCOL)
            with open('./optimizer/network_upgrade/topology_TM_{}_{}--{}.pkl'.format(tm, new_links[it_g][0],
                                                                                     new_links[it_g][1]), 'wb') as f:
                pickle.dump(G, f, pickle.HIGHEST_PROTOCOL)
            with open('./optimizer/network_upgrade/delays_TM_{}_{}--{}.pkl'.format(tm, new_links[it_g][0],
                                                                                   new_links[it_g][1]), 'wb') as f:
